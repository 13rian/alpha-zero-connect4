...
epoch_count = 150                        # the number of epochs to train the neural network
episode_count = 100                     # the number of games that are self-played in one epoch
update_count = 15                       # the number the neural net is updated  in one epoch with the experience data
...
temp_threshold = 18                     # up to this move the temp will be temp, otherwise 0 (deterministic play)
...
for i in range(epoch_count):
    ...
    if network_improved:
        ...
        logger.info("new generation network has improved, generation count: {}".format(gen_count))
        gen_count += 1
